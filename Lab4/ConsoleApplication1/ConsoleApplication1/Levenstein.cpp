...
int min3(int x1, int x2, int x3)  // âîçâğàùàåò ìèíèìàëüíîå çíà÷åíèå èç òğåõ
{
	return std::min(std::min(x1, x2), x3);
}

int levenshtein(int lx, const char x[], int ly, const char y[]) // ÄÈÍÀÌÈ×ÅÑÊÎÅ ÏĞÎÃĞÀÌÌÈĞÎÂÀÍÈÅ
{
...
int levenshtein_r(int lx, const char x[], int ly, const char y[]) // ĞÅÊÓĞÑÈß
{
	int rc = 0;
	if (lx == 0) rc = ly;
	else  if (ly == 0) rc = lx;
