...
int lcs(int lenx, const char x[], int leny, const char y[]) // –≈ ”–—»¬Õ€… Ã≈“Œƒ
{
	int rc = 0;
	if (lenx > 0 && leny > 0)
	{
...
void getLCScontent(int lenx, int leny, const char x[], const Dart* B, int n, int i, int j, char z[])
{
	if ((i > 0 && j  > 0 && n > 0))
	{
...
int lcsd(const char x[], const char y[], char z[]) // ƒ»Õ¿Ã»◊≈— Œ≈ œ–Œ√–¿ÃÃ»–Œ¬¿Õ»≈
{
	int n;
